...
function createFuzzyRegExp(keyword) {
  // 生成模糊匹配的正则表达式，忽略空格差异
  const fuzzyKeyword = keyword.split(' ').join('\\s*');
  return new RegExp(fuzzyKeyword, 'i');
}

function matchKeywords(documentText, keyword) {
  const preprocessedKeyword = preprocessKeyword(keyword);
  const regex = createFuzzyRegExp(preprocessedKeyword);
...
class PDFFindBar {
  ...
  dispatchEvent(type, findPrev = false) {
    var preprocessedQuery = preprocessKeyword(this.findField.value);
